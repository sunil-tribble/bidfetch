...
export class FPDSClient {
  ...
  private buildFeedUrl(params: {
    ...
    const baseUrl = config.apis.fpds.baseUrl;
  ...
  async fetchContracts(params: {
    ...
    await this.rateLimiter.waitForQuota({
      source: 'fpds',
      limit: config.apis.fpds.rateLimit,
      window: config.apis.fpds.rateWindow,
    });
    ...
    try {
      ...
      logger.error('Failed to fetch FPDS contracts', error);
      throw error;
    }
  }
  
  private parseEntry(entry: any): FPDSContract {
  ...
  async fetchAllContracts(params: {
    ...
    const allContracts: FPDSContract[] = [];
    let offset = 0;
    ...
    while (allContracts.length < maxRecords) {
      try {
        ...
        logger.info(`FPDS Progress: ${allContracts.length} contracts fetched`);
        
        // Small delay between requests
        await new Promise(resolve => setTimeout(resolve, 200));
        
      } catch (error) {
        logger.error(`Failed to fetch FPDS page at offset ${offset}`, error);
        break;
      }
    }
  ...
  async saveContract(contract: FPDSContract): Promise<void> {
  ...
  async analyzeContractExpiring(monthsAhead: number = 12): Promise<any[]> {
  ...
  async findIncumbent(agency: string, naicsCode: string): Promise<any> {
    const result = await this.db.query(`
      SELECT 
        c.*,
        o.name as contractor_name,
  ...
  async syncContracts(params: {
    ...
    logger.info('Starting FPDS sync...');
    ...
    try {
      ...
      for (const contract of contracts) {
        try {
          ...
          if (processedCount % 100 === 0) {
            logger.info(`Processed ${processedCount}/${contracts.length} contracts`);
          }
        } catch (error) {
          errorCount++;
          logger.error(`Failed to process contract ${contract.content.contractId}`, error);
        }
      }
      ...
      logger.info(`FPDS sync completed. Processed: ${processedCount}, Errors: ${errorCount}, Time: ${Date.now() - startTime}ms`);
