...
export class UKContractsClient {
  ...
  constructor() {
    this.axios = axios.create({
      baseURL: config.apis.ukContracts.baseUrl,
      headers: {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      const response = await axios.post(
        config.apis.ukContracts.tokenUrl || 'https://www.contractsfinder.service.gov.uk/token',
        new URLSearchParams({
          grant_type: 'client_credentials',
          client_id: config.apis.ukContracts.clientId,
          client_secret: config.apis.ukContracts.clientSecret,
        }),
        {
          headers: {
      ...
      logger.info('UK Contracts OAuth token refreshed');
      return this.accessToken;
      
    } catch (error) {
      logger.error('Failed to get UK Contracts access token', error);
  ...
  async searchContracts(params: {
    ...
    await this.rateLimiter.waitForQuota({
      source: 'uk_contracts',
      limit: config.apis.ukContracts.rateLimit,
      window: config.apis.ukContracts.rateWindow,
    });
    ...
    try {
      ...
      logger.info(`Fetched ${response.data.releases?.length || 0} UK contracts`);
      
      return response.data;
      
    } catch (error: any) {
      if (error.response?.status === 429) {
        logger.warn('UK Contracts rate limit hit');
        throw new Error('Rate limit exceeded');
      }
      
      logger.error('Failed to search UK contracts', error);
      throw error;
    }
  }
  ...
  async fetchAllContracts(params: {
    ...
    const allContracts: OCDSRelease[] = [];
    ...
    while (allContracts.length < maxRecords) {
      try {
        ...
        logger.info(`UK Contracts Progress: ${allContracts.length} contracts fetched`);
        
        // Rate limiting delay
        await new Promise(resolve => setTimeout(resolve, 200));
        
      } catch (error) {
        logger.error(`Failed to fetch UK contracts page at offset ${offset}`, error);
        break;
      }
    }
  ...
  private parseOCDSValue(value: any): number | null {
  ...
  async saveContract(release: OCDSRelease): Promise<void> {
    ...
    try {
      ...
      if (documents.length > 0) {
        const oppResult = await client.query(
          'SELECT id FROM opportunities WHERE source_id = $1 AND external_id = $2',
      ...
      logger.error('Failed to save UK contract', error);
      throw error;
    } finally {
      client.release();
    }
  ...
  async syncContracts(params: {
    ...
    logger.info('Starting UK Contracts sync...');
    ...
    try {
      ...
      logger.info(`Found ${contracts.length} UK contracts to process`);
      ...
      for (const contract of contracts) {
        try {
          ...
          if (processedCount % 50 === 0) {
            logger.info(`Processed ${processedCount}/${contracts.length} UK contracts`);
          }
        } catch (error) {
          errorCount++;
          logger.error(`Failed to process UK contract ${contract.ocid}`, error);
        }
      }
      ...
      logger.info(`UK Contracts sync completed. Processed: ${processedCount}, Errors: ${errorCount}, Time: ${Date.now() - startTime}ms`);
