...
class MLPredictionService {
    ...
    async initializeModel() {
        try {
            ...
            logger_1.logger.info('ML model initialized');
            // Load pre-trained weights if available
            await this.loadModelWeights();
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize ML model', error);
        }
    }
    async loadModelWeights() {
        try {
            const modelPath = `${config_1.config.ml.modelPath}/award-prediction-model.json`;
            // In production, load from S3 or model registry
            // this.model = await tf.loadLayersModel(modelPath);
            logger_1.logger.info('Model weights loaded');
        }
        catch (error) {
            logger_1.logger.warn('No pre-trained model found, using random initialization');
        }
    }
    async extractFeatures(opportunityId) {
      ...
      `, [opportunityId]);
            ...
            logger_1.logger.error('Failed to extract features', error);
            return null;
        }
    }
    async predictOpportunity(opportunityId) {
        try {
    ...
    async estimateCompetition(opportunityId) {
        const result = await this.db.query(`
    ...
    async predictValue(opportunityId) {
        const result = await this.db.query(`
    ...
    async predictAwardDate(opportunityId) {
        const result = await this.db.query(`
    ...
    async findSimilarOpportunities(opportunityId, limit = 5) {
        const result = await this.db.query(`
    ...
    async analyzeFactors(opportunityId) {
        const result = await this.db.query(`
      SELECT 
        o.*,
        a.name as agency_name,
    ...
    calculateConfidence(probability, competition) {
    ...
    async savePrediction(opportunityId, prediction) {
        await this.db.query(`
    ...
    `);
        ...
        logger_1.logger.info('Model training completed');
    }
    async close() {
        await this.db.end();
    }
