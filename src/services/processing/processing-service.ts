...
import { OpportunityProcessor } from './processors/opportunity-processor';
import { ContractProcessor } from './processors/contract-processor';
import { DataEnricher } from './enrichment/data-enricher';
import { ProcessingResult, ProcessingJob } from '../types/processing-types';
...
export class ProcessingService extends EventEmitter {
  ...
  private initializeQueues(): void {
    ...
    this.enrichmentQueue = new Queue('data-enrichment', redisConfig);
    this.analyticsQueue = new Queue('analytics-processing', redisConfig);
  ...
  private initializeProcessors(): void {
    this.processors.set('opportunity', new OpportunityProcessor(this.logger));
    this.processors.set('contract', new ContractProcessor(this.logger));
    this.processors.set('enricher', new DataEnricher(this.logger, this.databaseService));
  }
   ...
   */
  async processRawData(rawData: any): Promise<void> {
   ...
   */
  private async processOpportunities(job: Job<ProcessingJob>): Promise<ProcessingResult> {
   ...
   */
  private async processContracts(job: Job<ProcessingJob>): Promise<ProcessingResult> {
    const startTime = Date.now();
    const { data: jobData } = job.data;
   ...
   */
  private async processDocuments(job: Job<ProcessingJob>): Promise<ProcessingResult> {
    const startTime = Date.now();
    const { data: jobData } = job.data;
   ...
   */
  private async enrichData(job: Job): Promise<any> {
   ...
   */
  private async crossReferenceData(job: Job): Promise<any> {
    const { type, ids, sourceId } = job.data;
   ...
   */
  private async generatePredictions(job: Job): Promise<any> {
   ...
   */
  private async updateAnalytics(job: Job): Promise<any> {
   ...
   */
  private async addProcessingJob(job: ProcessingJob): Promise<void> {
   ...
   */
  private getJobPriority(job: ProcessingJob): number {
    // Higher priority for newer data and critical sources
    let priority = 0;
    
    if (job.sourceName.includes('SAM.gov')) priority += 10;
    if (job.dataType === 'opportunities') priority += 5;
    if (job.metadata?.urgent) priority += 20;
   ...
   */
  private determineDataType(rawData: any): string {
  ...
  async getProcessingStats(): Promise<{
    ...
    return {
      ...
      processors: Object.fromEntries(
        Array.from(this.processors.keys()).map(key => [key, 'active'])
      )
    };
  }
