...
exports.CompetitiveIntelligenceEngine = void 0;
...
class CompetitiveIntelligenceEngine {
    ...
    `, [opportunityId]);
        ...
        const incumbent = await this.findIncumbent(opportunity.agency_name, opportunity.naics_codes?.[0]);
        // Get top competitors
        const topCompetitors = await this.getTopCompetitors(opportunity.agency_name, opportunity.naics_codes?.[0]);
        // Analyze market
        const marketAnalysis = await this.analyzeMarket(opportunity.agency_name, opportunity.naics_codes?.[0]);
        // Find expiring contracts
        const expiringContracts = await this.findExpiringContracts(opportunity.agency_name, opportunity.naics_codes?.[0]);
        // Calculate success factors
        const successFactors = await this.calculateSuccessFactors(opportunity.agency_name, opportunity.naics_codes?.[0]);
        // Store the analysis
    ...
    async getTopCompetitors(agency, naicsCode) {
        const result = await this.db.query(`
      WITH market_data AS (
        SELECT 
          c.contractor_id,
    ...
    async analyzeMarket(agency, naicsCode) {
        const result = await this.db.query(`
      SELECT 
    ...
    async findExpiringContracts(agency, naicsCode) {
        const result = await this.db.query(`
      SELECT 
        c.contract_id,
        o.name as contractor_name,
    ...
    async calculateSuccessFactors(agency, naicsCode) {
        const result = await this.db.query(`
    ...
    async saveAnalysis(opportunityId, analysis) {
        await this.db.query(`
    ...
    async predictRecompete(monthsAhead = 12) {
        const result = await this.db.query(`
    ...
    async generateCompetitorProfile(contractorId) {
...
exports.CompetitiveIntelligenceEngine = CompetitiveIntelligenceEngine;
