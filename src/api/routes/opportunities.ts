...
try {
  ...
  logger.warn('Database connection failed, will use JSON file fallback');
}

// Load opportunities from JSON file
const loadOpportunitiesFromFile = (): any[] => {
  try {
    ...
    logger.error('Failed to load opportunities from file', error);
    return [];
  }
};

// Transform normalized opportunity to API format
const transformOpportunity = (opp: any) => {
  // Transform resource links to document objects
  const documents = (opp.raw?.resourceLinks || []).map((link: string, index: number) => {
  ...
  try {
    ...
    if (db) {
      try {
        ...
        logger.warn('Database query failed, falling back to JSON', dbError);
  ...
  try {
    ...
    if (db) {
      try {
        ...
        logger.warn('Database query failed for opportunity', dbError);
      }
    }
    ...
    if (db) {
      try {
        const sourceResult = await db.query(
          'SELECT * FROM opportunities WHERE id = $1 OR external_id = $1',
          [id]
        );
        ...
        logger.warn('Database query failed for similar opportunities', dbError);
      }
    }
      ...
      if (sourceOpp) {
        similarOpportunities = opportunities
          ...
          .map(opp => {
            ...
            if (sourceNaics.some((naics: any) => oppNaics.includes(naics))) score += 0.3;
...
router.get('/:id/documents/:docId', async (req, res): Promise<void> => {
  try {
    ...
    const document = transformed.documents.find((doc: any) => doc.id === docId);
...
router.get('/:id/documents', async (req, res): Promise<void> => {
  try {
    const { id } = req.params;
...
router.get('/export/:format', async (req, res): Promise<void> => {
  try {
    ...
    if (format === 'csv') {
      ...
      const csvRows = opportunities.map(opp => 
        `"${opp.id}","${opp.title}","${opp.agency_name}","${opp.posted_date}","${opp.response_deadline || ''}","${opp.estimated_value || ''}","${opp.status}","${opp.ui_link}"`
      ).join('\n');
...
router.post('/search/advanced', async (req, res): Promise<void> => {
  try {
    const {
      keywords,
      agencies,
    ...
    if (keywords && keywords.length > 0) {
      opportunities.forEach(opp => {
        ...
        keywords.forEach((keyword: string) => {
          ...
          if (opp.office?.toLowerCase().includes(lowerKeyword)) score += 2;
        });
    ...
    if (naics_codes && naics_codes.length > 0) {
      opportunities = opportunities.filter(opp => 
        opp.naics_codes?.some((code: string) => naics_codes.includes(code))
      );
    }
    ...
    if (posted_after) {
      opportunities = opportunities.filter(opp => 
        new Date(opp.posted_date) >= new Date(posted_after)
      );
    }
    
    if (posted_before) {
      opportunities = opportunities.filter(opp => 
        new Date(opp.posted_date) <= new Date(posted_before)
      );
    }
    
    if (response_after) {
      opportunities = opportunities.filter(opp => 
        new Date(opp.response_deadline) >= new Date(response_after)
      );
    }
    
    if (response_before) {
      opportunities = opportunities.filter(opp => 
        new Date(opp.response_deadline) <= new Date(response_before)
      );
    }
...
router.get('/filters', async (_req, res): Promise<void> => {
  try {
    const opportunities = loadOpportunitiesFromFile().map(transformOpportunity);
    ...
    const documentTypes = [...new Set(opportunities.flatMap(opp => 
      (opp.documents || []).map((doc: any) => doc.type)
    ))];
...
router.get('/dashboard', async (req, res): Promise<void> => {
  try {
    ...
    const recentOpportunities = opportunities.filter(opp => 
      new Date(opp.posted_date) >= cutoffDate
    );
  ...
  try {
    ...
    const scoredOpportunities = opportunities
      ...
      .map(opp => {
        ...
        if (highActivityAgencies.includes(opp.agency_name || '')) score += 15;
        
        // Document availability
        if (opp.documents && opp.documents.length > 3) score += 10;
...
router.get('/stats', async (req, res): Promise<void> => {
  try {
    ...
    opportunities.forEach(opp => {
      ...
      else if (value < 1000000) valueRanges['$100K - $1M']++;
      else if (value < 10000000) valueRanges['$1M - $10M']++;
      else if (value < 100000000) valueRanges['$10M - $100M']++;
      else valueRanges['Over $100M']++;
    });
