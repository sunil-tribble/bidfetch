...
interface TEDNotice {
  ...
  AC: string; // Authority/Contracting Body
  TY: string; // Type of contract
  NC: string; // NUTS code (geographic)
  PR: string; // Procedure
  TD: string; // Document Type
  RP: string; // Regulation
  TV: string; // Total Value
  CY: string; // Country
  AA: string; // Awarding Authority Type
...
export class TEDEuropaClient {
  ...
  constructor() {
    ...
    if (config.apis.tedEu.apiKey) {
      this.axios.defaults.headers.common['Authorization'] = `Bearer ${config.apis.tedEu.apiKey}`;
    }
  ...
  async searchNotices(params: {
    ...
    await this.rateLimiter.waitForQuota({
      source: 'ted_eu',
      limit: config.apis.tedEu.rateLimit,
      window: config.apis.tedEu.rateWindow,
    });
    ...
    try {
      ...
      logger.info(`Fetched ${response.data.notices?.length || 0} notices from TED Europa`);
      ...
      if (error.response?.status === 429) {
        logger.warn('TED Europa rate limit hit');
        throw new Error('Rate limit exceeded');
      }
      
      logger.error('Failed to search TED notices', error);
      throw error;
    }
  }
  
  async fetchNoticeDetails(noticeId: string): Promise<any> {
    await this.rateLimiter.waitForQuota({
      source: 'ted_eu',
      limit: config.apis.tedEu.rateLimit,
      window: config.apis.tedEu.rateWindow,
    ...
    try {
      ...
      logger.error(`Failed to fetch notice details for ${noticeId}`, error);
      throw error;
    }
  }
  ...
  async fetchAllNotices(params: {
    ...
    while (allNotices.length < maxRecords) {
      try {
        ...
        logger.info(`TED Progress: ${allNotices.length}/${response.total} notices`);
        ...
        logger.error(`Failed to fetch TED page ${page}`, error);
        break;
      }
    }
  ...
  private parseCPVCodes(cpvString: string): string[] {
    if (!cpvString) return [];
  ...
  private mapCountryToISO(country: string): string {
  ...
  async saveNotice(notice: TEDNotice): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to save TED notice', error);
      throw error;
    } finally {
      client.release();
    }
  ...
  private mapContractType(tedType: string): string {
  ...
  async syncNotices(params: {
    ...
    logger.info('Starting TED Europa sync...');
    ...
    try {
      ...
      logger.info(`Found ${notices.length} TED notices to process`);
      ...
      for (const notice of notices) {
        try {
          ...
          if (processedCount % 50 === 0) {
            logger.info(`Processed ${processedCount}/${notices.length} TED notices`);
          }
        } catch (error) {
          errorCount++;
          logger.error(`Failed to process TED notice ${notice.ND}`, error);
        }
      }
      ...
      logger.info(`TED Europa sync completed. Processed: ${processedCount}, Errors: ${errorCount}, Time: ${Date.now() - startTime}ms`);
      
    } catch (error) {
      logger.error('TED Europa sync failed', error);
      throw error;
    }
  }
